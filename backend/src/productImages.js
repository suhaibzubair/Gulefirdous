...
function resolvePhotoPoolForCategory(category = "Perfume") {
...
function getPhotoPoolSize(category = "Perfume") {
...
function toImageOption(photo, token, index, category, labelSuffix = "") {
...
function createRealisticImageOptions(productName, seed = Date.now(), category = "Perfume") {
...
function createNextImageBatch(
  ...
  const productKey = (productName || "gulefirdous-product").trim().toLowerCase();
...
module.exports = {
  ...
  resolvePhotoPoolForCategory,
  totalPhotoPoolSize: PERFUME_PHOTOS.length,
};
